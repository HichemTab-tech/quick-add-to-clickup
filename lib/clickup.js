...
export class ClickUpClient {
  constructor(token, { fetchImpl = globalThis.fetch, baseUrl = DEFAULT_BASE_URL } = {}) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new ClickUpError("This browser does not provide the Fetch API.");
    }
    this.token = token.trim();
    // Native WorkerGlobalScope.fetch requires its receiver to be the worker global.
    // Calling a detached native fetch as `client.fetch()` causes "Illegal invocation" in Chrome.
